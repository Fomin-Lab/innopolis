    ...
    private String getRandomWord() {
        if (probability != null && probability.isLoaded()) {
            ...
            /*
                ...
                например: слово в предыдущем абзаце имеет вероятность повторится в след. абзаце с вероятность 50%, то
                если сгенерированное случайное число попадает в интервал от 0 до 50 - то считаем что возникло событие и добавляем
                это слово в предложение, иначе считаем что события не возникло и генерируем новое слово.
            ...
            if (probabilityEntry != null && new Random().nextInt(100 + 1) <= probabilityEntry.getValue()) {
                // вызываем toLowerCase т.к. сохраненное слово могла стоять в начале абзаца
